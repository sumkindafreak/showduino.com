...
(function () {
  ...
  async function renderProjects(user) {
    ...
    cloud.forEach((project) => {
      ...
      item.innerHTML = `<div><strong>${escapeHtml(project.name || 'Untitled Show')}</strong><div class="muted">Updated ${escapeHtml(new Date(project.updated_at).toLocaleString())}</div></div><span class="badge">SUPABASE</span>`;
      ...
      item.addEventListener('keydown',(event)=>{ if(event.key==='Enter'||event.key===' ') item.click(); });
    ...
    local.forEach((project) => {
      ...
      item.addEventListener('click', () => {
        sessionStorage.setItem('showduino_open_local_project', project.id);
        window.location.href = 'studio.html';
      });
      item.tabIndex = 0;
      item.setAttribute('role','button');
  ...
  function fillProfile(profile, user) {
    elements.profileDisplayName.value = profile?.display_name || user?.user_metadata?.display_name || 'Showduino Creator';
    elements.profileHauntName.value = profile?.haunt_name || '';
  ...
  async function showSignedIn(user) {
    ...
    try {
      profile = await window.ShowduinoSupabase.getProfile();
    ...
    setStatus(confirmationReturn ? 'Email confirmed — welcome to Showduino. Your Showduino ID is ready.' : 'Signed in. HauntSync and your cloud shows are ready.', 'success');
  ...
  async function handleSignIn(event) {
    ...
    try { await window.ShowduinoSupabase.signIn(elements.loginEmail.value.trim(), elements.loginPassword.value); }
    catch(error){ setStatus(friendlyAuthMessage(error,'Sign in failed.'),'error'); }
  }
  ...
  async function handleRecoverySubmit(event) {
    ...
    try {
      ...
      setStatus(friendlyAuthMessage(error,'Your password could not be updated.'),'error');
    }
  }

  async function handleProfileSubmit(event) {
    event.preventDefault();
    setStatus('Saving your HauntSync identity…');
    try {
      const profile = await window.ShowduinoSupabase.updateProfile({
        displayName: elements.profileDisplayName.value,
        hauntName: elements.profileHauntName.value,
        bio: elements.profileBio.value
      });
      elements.accountName.textContent = profile.display_name || 'Showduino Creator';
      fillProfile(profile, await window.ShowduinoSupabase.getCurrentUser());
      setStatus('HauntSync identity updated.', 'success');
    } catch (error) {
      setStatus(friendlyAuthMessage(error, 'Your HauntSync identity could not be saved.'), 'error');
    }
  }
  ...
  function initialise() {
    ...
    elements.profilePanel = byId('profile-panel');
    elements.profileForm = byId('profile-form');
    elements.profileDisplayName = byId('profile-display-name');
    elements.profileHauntName = byId('profile-haunt-name');
    elements.profileBio = byId('profile-bio');
    ...
    window.ShowduinoSupabase.onAuthChanged(async(user,event) => {
      ...
      if (user) {
        ...
        showSignedOut();
        setStatus(confirmationReturn ? 'Email confirmed. Sign in to finish opening your Showduino ID.' : 'Sign in or create a free Showduino account.', confirmationReturn ? 'success' : undefined);
        await renderProjects(null);
      }
    });
  }
